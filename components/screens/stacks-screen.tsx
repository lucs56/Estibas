...
export default function StacksScreen(props: Props) {
  ...
  const grouped=useMemo(()=>{const map=new Map<string,EvaluatedStack[]>();filtered.forEach(stack=>{const key=stockGroupKey(stack.productCode,stack.lot,stack.elaborationDate,stack.product,stack.cut);map.set(key,[...(map.get(key)??[]),stack]);});return [...map.entries()].map(([key,members])=>({key,members,first:members[0],original:members.reduce((n,item)=>n+item.originalQuantity,0),available:members.reduce((n,item)=>n+item.availableQuantity,0),liters:members.reduce((n,item)=>n+numericExtra(item,"Litros"),0)}));},[filtered]);
  ...
  return <>
    ...
    <section className="panel stacks-panel">
      ...
      <div className="consumption-list">{visible.flatMap(group=>{const items=(group.first.extraData.consumptions as Array<{requestNumber:string;pn:string;bottles:number}>|undefined)??[];return items.map(item=><div key={`${group.key}-${item.requestNumber}-${item.pn}`}><strong>{group.first.productCode} · {group.first.product} · {group.first.lot}</strong><span>Ya ocupaste {item.bottles.toLocaleString("es-AR")} botellas para el pedido {item.pn} ({item.requestNumber}). El saldo mostrado arriba ya está descontado.</span></div>);})}</div>
