...
import { Download, FileDown, History, RefreshCw, Search, TestTube2, Trash2 } from "lucide-react";
...
type Props={requests:VeRequest[];onRequests:(value:VeRequest[])=>void;onDelete:(request:VeRequest)=>void;onRefresh:()=>void};

export default function HistoryScreen({requests,onRequests,onDelete,onRefresh}:Props) {
  ...
  return <>
    <section className="page-heading"><div><div className="eyebrow"><i/> Trazabilidad documental</div><h1>Historial de Solicitudes VE</h1><p>Compartido entre todos los usuarios. Se actualiza automáticamente cada 10 segundos.</p></div><div className="heading-actions"><button className="secondary" onClick={onRefresh}><RefreshCw size={17}/> Actualizar historial</button><button className="secondary" disabled={!selected.length} onClick={exportSamples}><TestTube2 size={17}/> Reporte de muestras ({selected.length})</button><button className="primary" disabled={!selected.length} onClick={()=>void exportWeeklyVeExcel(selected)}><Download size={17}/> Generar Solicitud VE semanal ({selected.length})</button></div></section>
