...
export default function OrdersScreen({orders,onSync,onSelect,busy}:{orders:ProductionOrder[];onSync:()=>Promise<void>;onSelect:(order:ProductionOrder)=>void;busy:boolean}) {
  ...
  return <>
    ...
    <section className="connection-card"><div className="connection-icon"><Cloud size={23}/></div><div><span className="connection-status"><i/> INSTANTÁNEA VALIDADA</span><strong>Programación Junín · Google Sheets</strong><p>Lectura por secciones “PROGRAMACIÓN LÍNEA 1/2/3”. Las actividades FRACCIONAR no ingresan en esta pantalla.</p></div><div className="connection-meta"><span>Filas disponibles</span><strong>{dressingOrders.length} para vestir</strong><small>ID …xeBDPQ</small></div></section>
