...
import { CheckCircle2, Cloud, FileSpreadsheet, RefreshCw, Search, Sparkles } from "lucide-react";
import { useEffect, useMemo, useRef, useState } from "react";
import type { ProductionOrder } from "@/lib/types";

const lines=[1,2,3] as const;

export default function OrdersScreen({orders,onSync,onSelect,onToggle,busy}:{orders:ProductionOrder[];onSync:()=>Promise<void>;onSelect:(order:ProductionOrder)=>void;onToggle:(id:string)=>void;busy:boolean}) {
  ...
  return <>
    ...
    <section className="panel order-panel">
      ...
      <div className="table-scroll"><table className="orders-table program-table"><thead><tr><th>Hecha</th><th>PIN°</th><th>Código</th><th>Marca / Variedad</th><th>Cosecha / Capacidad</th><th>Cliente / País destino</th><th>Cajas × Cj x</th><th>Botellas</th><th>Observaciones / FRC</th><th></th></tr></thead><tbody>{visible.map(order=><tr key={order.id} className={order.veCompleted?"order-completed":order.highlightedNew?"order-new":""}><td><button className="order-check" title="Marcar solicitud realizada" onClick={()=>onToggle(order.id)}><CheckCircle2 size={20}/></button></td><td><strong className="mono">{order.pn}</strong><small>Fila {order.sheetRow}</small></td><td><strong className="mono">{order.internalCode}</strong><small>{order.closure}</small></td><td><strong>{order.brand}</strong><small>{order.variety}</small></td><td><strong>{order.harvest}</strong><small>{order.capacity}</small></td><td><strong>{order.client}</strong><small>{order.country}</small></td><td><strong>{order.boxes.toLocaleString("es-AR")} × {order.unitsPerBox}</strong><small>{order.presentation}</small></td><td><strong>{order.bottles.toLocaleString("es-AR")}</strong><small>VESTIR</small></td><td><strong>{order.observations||"—"}</strong><small>FRC {order.frc||"—"}</small></td><td><button className="quiet" onClick={()=>onSelect(order)}><Sparkles size={14}/> Elegir producto</button></td></tr>)}</tbody></table></div>
